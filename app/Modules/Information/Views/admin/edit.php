...
<div class="container-fluid p-0">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h4 class="text-body font-heading m-0 fw-bold">Edit Informasi</h4>
                <p class="text-secondary style-tiny m-0">Perbarui data atau status pengumuman</p>
            </div>
        </div>
    </div>
    ...
    <div class="saas-card p-4 border border-secondary border-opacity-25 bg-body-tertiary">
        <form action="<?= base_url('admin/informasi/update/' . $info['id']) ?>" method="POST">
            <?= csrf_field() ?>
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-8">
                    ...
                    <input type="text" name="title" class="form-control bg-body border-secondary border-opacity-50 text-body style-tiny" value="<?= old('title', $info['title']) ?>" required>
                </div>
                ...
                <div class="col-12 col-md-4">
                    ...
                    <select name="category" class="form-select bg-body border-secondary border-opacity-50 text-body style-tiny" required>
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= esc($cat) ?>" <?= old('category', $info['category']) === $cat ? 'selected' : '' ?>>
                                <?= esc($cat) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="datetime-local" name="date_time" class="form-control bg-body border-secondary border-opacity-50 text-body style-tiny" value="<?= old('date_time', date('Y-m-d\TH:i', strtotime($info['date_time']))) ?>" required>
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="status" class="form-select bg-body border-secondary border-opacity-50 text-body style-tiny">
                        <option value="active" <?= old('status', $info['status']) === 'active' ? 'selected' : '' ?>>Aktif (Tampilkan)</option>
                        <option value="archived" <?= old('status', $info['status']) === 'archived' ? 'selected' : '' ?>>Arsip (Sembunyikan)</option>
                    </select>
                </div>
                ...
                <div class="col-12">
                    <div class="p-3 rounded-3 bg-body border border-secondary border-opacity-25 d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <small class="text-secondary style-tiny d-block mt-0.5">Jika diaktifkan, informasi ini akan muncul sebagai popup modal di layar anggota sampai mereka mengklik tombol <strong>"Sudah Dibaca"</strong>.</small>
                        </div>
                        <div class="form-check form-switch m-0 ms-3">
                            <input class="form-check-input cursor-pointer" type="checkbox" name="is_popup" value="1" id="is_popup_switch" <?= old('is_popup', $info['is_popup']) ? 'checked' : '' ?> style="width: 2.5em; height: 1.3em;">
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" rows="7" class="form-control bg-body border-secondary border-opacity-50 text-body style-tiny" required><?= old('description', $info['description']) ?></textarea>
                </div>
            </div>
