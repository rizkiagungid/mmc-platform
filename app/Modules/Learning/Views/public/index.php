...
<section class="py-5">
    <div class="container py-4">
        ...
        <div class="text-center mb-5">
            ...
            <h1 class="display-5 fw-bold text-body font-heading mt-2">Pusat Materi Pembelajaran & Panduan Digital</h1>
            ...
            <form action="<?= base_url('materi') ?>" method="GET" class="col-lg-8 mx-auto mt-4">
                <div class="input-group input-group-lg shadow-lg">
                    <span class="input-group-text bg-body-secondary border-secondary border-opacity-50 text-secondary"><i class="fa-solid fa-magnifying-glass text-danger"></i></span>
                    <input type="text" name="q" class="form-control bg-body-secondary text-body border-secondary border-opacity-50" placeholder="Cari materi pembelajaran, topik, atau kata kunci..." value="<?= esc($filters['search'] ?? '') ?>">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-center gap-2 mt-4">
                <a href="<?= base_url('materi') ?>" class="badge p-2 px-3 <?= empty($filters['division_id']) ? 'bg-danger text-white' : 'bg-body-secondary text-secondary border border-secondary border-opacity-50' ?> text-decoration-none font-monospace">
                    Semua Divisi
                </a>
                <?php foreach ($divisions as $d): ?>
                    <a href="<?= base_url('materi?div=' . $d['id']) ?>" class="badge p-2 px-3 <?= ($filters['division_id'] ?? '') == $d['id'] ? 'bg-danger text-white' : 'bg-body-secondary text-secondary border border-secondary border-opacity-50' ?> text-decoration-none font-monospace">
                        <?= esc($d['name']) ?>
                    </a>
                <?php endforeach; ?>
            </div>
        ...
        <?php if (!empty($featured) && empty($filters['search']) && empty($filters['division_id'])): ?>
            <div class="mb-5">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <h4 class="text-body font-heading fw-bold m-0"><i class="fa-solid fa-star text-warning me-2"></i> Materi Pembelajaran Unggulan (Featured)</h4>
                </div>
                <div class="row g-4">
                    <?php foreach ($featured as $f): ?>
                        <div class="col-lg-4">
                            <div class="saas-card saas-card-glow h-100 d-flex flex-column justify-content-between overflow-hidden border border-secondary border-opacity-25">
                                <div>
                                    <div class="position-relative bg-body-secondary overflow-hidden" style="height: 180px;">
                                        ...
                                        <?php else: ?>
                                            <div class="w-100 h-100 bg-body-secondary d-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.1), rgba(59, 130, 246, 0.1));">
                                    ...
                                    <div class="p-4">
                                        <div class="d-flex align-items-center justify-content-between mb-2">
                                            ...
                                            <span class="badge bg-body-secondary border border-secondary border-opacity-50 text-body font-monospace"><?= esc($f['category']) ?></span>
                                        </div>
                                        <h5 class="text-body font-heading mb-2 fw-bold">
                                            <a href="<?= base_url('materi/' . $f['slug']) ?>" class="text-body text-decoration-none hover-danger"><?= esc($f['title']) ?></a>
                                        </h5>
                                        <p class="text-secondary small mb-3"><?= esc(mb_strimwidth($f['excerpt'] ?? '', 0, 100, '...')) ?></p>
                                    </div>
                                </div>
                                <div class="p-4 pt-0 border-top border-secondary border-opacity-25 mt-auto d-flex align-items-center justify-content-between text-secondary style-tiny font-monospace pt-3">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h4 class="text-body font-heading fw-bold m-0"><i class="fa-solid fa-book-open text-danger me-2"></i> Daftar Materi Pembelajaran</h4>
        ...
        <div class="row g-4">
            ...
            <?php foreach ($materials as $m): ?>
                <div class="col-md-6 col-lg-4">
                    <div class="saas-card saas-card-glow h-100 d-flex flex-column justify-content-between overflow-hidden border border-secondary border-opacity-25">
                        <div>
                            <div class="position-relative bg-body-secondary overflow-hidden" style="height: 190px;">
                            ...
                            <div class="p-4">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    ...
                                    <span class="badge bg-body-secondary border border-secondary border-opacity-50 text-body font-monospace"><?= esc($m['category']) ?></span>
                                </div>
                                <h5 class="text-body font-heading mb-2 fw-bold">
                                    <a href="<?= base_url('materi/' . $m['slug']) ?>" class="text-body text-decoration-none hover-danger"><?= esc($m['title']) ?></a>
                                </h5>
                                <p class="text-secondary small mb-3"><?= esc(mb_strimwidth($m['excerpt'] ?? '', 0, 110, '...')) ?></p>
                            </div>
                        </div>

                        <div class="p-4 pt-0 border-top border-secondary border-opacity-25 mt-auto">
