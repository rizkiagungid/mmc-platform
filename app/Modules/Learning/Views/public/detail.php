...
<section class="py-5">
    <div class="container py-4">
        ...
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb font-monospace style-tiny">
                <li class="breadcrumb-item"><a href="<?= base_url('/') ?>" class="text-secondary text-decoration-none"><i class="fa-solid fa-house me-1"></i> Home</a></li>
                <li class="breadcrumb-item"><a href="<?= base_url('materi') ?>" class="text-secondary text-decoration-none">Learning Center</a></li>
                <li class="breadcrumb-item"><a href="<?= base_url('materi?cat=' . urlencode($material['category'])) ?>" class="text-secondary text-decoration-none"><?= esc($material['category']) ?></a></li>
                <li class="breadcrumb-item active text-danger text-truncate" style="max-width: 300px;"><?= esc($material['title']) ?></li>
            </ol>
        </nav>
        ...
        <div class="row g-4 justify-content-center">
            <div class="col-lg-9">
                <!-- Main Article Card -->
                <div class="saas-card overflow-hidden p-4 p-md-5">
                    <!-- Banner Image (If Any) -->
                    ...
                    <div class="mb-4 pb-3 border-bottom border-secondary border-opacity-25">
                        ...
                        <h1 class="display-6 fw-bold text-white font-heading mb-3"><?= esc($material['title']) ?></h1>
                        ...
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 pt-2">
                            <div class="d-flex align-items-center gap-3">
                                <?php if (!empty($material['author_avatar'])): ?>
                                    <img src="<?= base_url($material['author_avatar']) ?>" alt="Author" class="rounded-circle object-fit-cover border border-danger border-2" style="width: 46px; height: 46px;">
                                <?php else: ?>
                                    <div class="rounded-circle bg-danger bg-opacity-25 text-danger fw-bold d-flex align-items-center justify-content-center border border-danger border-opacity-50" style="width: 46px; height: 46px;">
                    ...
                    <div class="pt-4 border-top border-secondary border-opacity-25 d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <?php if (!empty($material['tags'])): ?>
                                <?php foreach ($material['tags'] as $t): ?>
                                    <a href="<?= base_url('materi?tag=' . $t['slug']) ?>" class="badge bg-dark border border-secondary text-secondary text-decoration-none font-monospace style-tiny">#<?= esc($t['name']) ?></a>
                                <?php endforeach; ?>
                            <?php else: ?>
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <?php $currentFullUrl = current_url(); ?>
                            ...
                            <a href="https://api.whatsapp.com/send?text=<?= urlencode($material['title'] . ' ' . $currentFullUrl) ?>" target="_blank" class="btn btn-sm btn-outline-success" title="Bagikan ke WhatsApp">
                ...
                <?php if (!empty($related)): ?>
                    <div class="mt-5">
                        <h4 class="text-white font-heading fw-bold mb-4"><i class="fa-solid fa-layer-group text-danger me-2"></i> Materi Pembelajaran Terkait</h4>
                        <div class="row g-4">
                            <?php foreach ($related as $rel): ?>
                                <div class="col-md-6">
                                    <div class="saas-card saas-card-glow h-100 p-4 d-flex flex-column justify-content-between">
                                        <div>
                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                <span class="badge bg-danger font-monospace"><?= esc($rel['division_name'] ?: 'Umum') ?></span>
                                                <span class="badge bg-secondary font-monospace"><?= esc($rel['category']) ?></span>
                                            </div>
                                            <h6 class="text-white font-heading mb-2">
                                                <a href="<?= base_url('materi/' . $rel['slug']) ?>" class="text-white text-decoration-none hover-danger"><?= esc($rel['title']) ?></a>
                                            </h6>
                                            <p class="text-secondary small mb-3"><?= esc(mb_strimwidth($rel['excerpt'] ?? '', 0, 90, '...')) ?></p>
                                        </div>
                                        <div class="pt-2 border-top border-secondary border-opacity-10 d-flex align-items-center justify-content-between text-secondary style-tiny font-monospace">
                                            ...
                                            <a href="<?= base_url('materi/' . $rel['slug']) ?>" class="text-danger fw-bold text-decoration-none">Baca <i class="fa-solid fa-arrow-right"></i></a>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
