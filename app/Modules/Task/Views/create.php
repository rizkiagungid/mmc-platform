...
<div class="saas-card p-3 p-md-4 col-12 col-lg-9 mx-auto">
    <form action="<?= base_url('admin/tasks/store') ?>" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control form-control-lg bg-black text-white border-secondary border-opacity-50" placeholder="Contoh: Aftermovie MPLS SMAN 1 Tamansari 2026" required value="<?= old('title') ?>">
        </div>

        <div class="mb-3">
            <label class="form-label text-secondary small fw-medium">Deskripsi & Instruksi Pengerjaan</label>
            <textarea name="description" class="form-control bg-black text-white border-secondary border-opacity-50" rows="4" placeholder="Jelaskan detail instruksi, ketentuan pengerjaan tugas, atau tautan aset..."><?= old('description') ?></textarea>
        </div>
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-12 col-md-4">
                ...
                <input type="datetime-local" name="deadline" class="form-control bg-black text-white border-secondary border-opacity-50" value="<?= old('deadline') ?>">
            </div>
        </div>
        ...
        <div class="mb-4">
            <div class="d-flex align-items-center justify-content-between mb-2 flex-wrap gap-2">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <label class="form-label text-white fw-semibold m-0">
                        <i class="fa-solid fa-users text-danger me-2"></i> Pilih Anggota Assignee <span class="text-secondary style-tiny fw-normal">(Opsional)</span>
                    </label>
            ...
            <div class="p-2 p-sm-3 rounded-3 bg-dark border border-secondary border-opacity-25 custom-scroll-container" style="max-height: 300px; overflow-y: auto;">
                <div class="row g-2" id="assignee-list-container">
                    <?php foreach ($members as $m): ?>
                        ...
                        ?>
                        <div class="col-12 col-sm-6 col-lg-4 assignee-item" data-division="<?= $division ?>" data-search="<?= esc(strtolower($m['full_name'] . ' ' . $m['nis_nip'] . ' ' . $m['class_dept'])) ?>">
                            <div class="form-check p-2 rounded-2 border border-secondary border-opacity-10 bg-black h-100">
                                <input class="form-check-input ms-1 assignee-checkbox" type="checkbox" name="assignees[]" value="<?= $m['id'] ?>" id="assignee_<?= $m['id'] ?>" data-name="<?= esc($m['full_name']) ?>" data-dept="<?= esc($m['class_dept'] ?: 'Anggota') ?>" <?= (is_array(old('assignees')) && in_array($m['id'], old('assignees'))) ? 'checked' : '' ?>>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.btn-filter-division').on('click', function() {
            ...
            $(`.assignee-item[data-division="${targetDivision}"] .assignee-checkbox`).prop('checked', true).trigger('change');
        });
